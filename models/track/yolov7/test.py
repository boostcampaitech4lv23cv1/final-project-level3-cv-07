...
def test(
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            if flip_test:
                ...
                out_flip, train_out_flip = model(
                    img_flip, augment=augment
                )  # inference and training outputs
                model.model[-1].flip_test = False
            ...
            if kpt_label:
                ...
                targets[:, 2:] *= torch.Tensor([width, height] * num_points).to(
                    device
                )  # to pixels
            else:
    ...
    if save_json or save_json_kpt and len(jdict):
        ...
        if save_json:
            ...
            anno_json = (
                "../coco/annotations/person_keypoints_val2017.json"  # annotations json
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--flip-test", action="store_true", help="Whether to run flip_test or not"
    )
