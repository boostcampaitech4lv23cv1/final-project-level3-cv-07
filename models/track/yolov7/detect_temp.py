...
def detect_image(opt):
    ...
    half = (
        device.type != "cpu" and not save_txt_tidl
    ...
    for _, path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                scale_coords(
                    img.shape[2:], det[:, 6:], im0.shape, kpt_label=kpt_label, step=3
                )
                ...
                if save_txt_tidl:  # Write to file in tidl dump format
                    for *xyxy, conf, cls in det_tidl:
                        ...
                        line = (
                            (conf, cls, *xyxy) if opt.save_conf else (cls, *xyxy)
                        )  # label format
                        with open(txt_path + ".txt", "a") as f:
                            f.write(("%g " * len(line)).rstrip() % line + "\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        default="/opt/ml/BoT-SORT/pretrained/yolov7-tiny.pt",
        help="model.pt path(s)",
    )
    parser.add_argument(
        "--source", type=str, default="/opt/ml/BoT-SORT/target", help="source"
