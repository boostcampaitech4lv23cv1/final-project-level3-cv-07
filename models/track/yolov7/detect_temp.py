...
def detect_image(opt):
    source, weights, view_img, save_txt, imgsz, save_txt_tidl, kpt_label = opt.target, opt.weights, opt.view_img, opt.save_txt, 640, opt.save_txt_tidl, opt.kpt_label
    ...
    t0 = time.time()
    for _,path, img, im0s, vid_cap in dataset:
        ...
        print(pred[...,4].max())
        # Apply NMS
        pred = non_max_suppression(pred, 0.25, 0.45, classes=opt.classes, agnostic=opt.agnostic_nms, kpt_label=kpt_label)
    ...
    return features_keep, saver_dir
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='/opt/ml/BoT-SORT/target', help='source')  # file/folder, 0 for webcam
