...
def get_parser():
    ...
    parser.add_argument(
        "--model-path", default="onnx_model/baseline.onnx", help="onnx model path"
    )
    parser.add_argument(
        "--input",
        nargs="+",
    ...
    parser.add_argument(
        "--output", default="onnx_output", help="path to save converted caffe model"
    )
...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        for path in tqdm.tqdm(args.input):
            ...
            np.save(
                os.path.join(args.output, path.replace(".jpg", ".npy").split("/")[-1]),
                feat,
            )
