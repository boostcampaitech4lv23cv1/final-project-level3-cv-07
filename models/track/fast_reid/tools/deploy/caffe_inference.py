...
def get_parser():
    ...
    parser.add_argument(
        "--output", default="caffe_output", help="path to save converted caffe model"
    )
...
def preprocess(image_path, image_height, image_width):
    ...
    image = (
        image - np.array([0.485 * 255, 0.456 * 255, 0.406 * 255]).reshape((1, -1, 1, 1))
    ) / np.array([0.229 * 255, 0.224 * 255, 0.225 * 255]).reshape((1, -1, 1, 1))
    return image
